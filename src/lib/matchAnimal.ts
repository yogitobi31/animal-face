...
export function matchAnimal(v: EmotionVector): MatchResult {
  const ranked=animalCards.map((card)=>{
    const d=Math.sqrt(Object.entries(w).reduce((s,[k,weight])=>{const diff=v[k as keyof EmotionVector]-card.featureWeights[k as keyof EmotionVector];return s+weight*diff*diff},0))
